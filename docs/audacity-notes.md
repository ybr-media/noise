...
stereo white-noise stem. The unfiltered project was saved, the bell-only
`FilterCurve:` was applied, and the filtered project was saved and decoded
through the AUP3 serializer. The measured response was the Welch-spectrum
ratio of filtered to unfiltered audio.

The intended analytic response is the standard peaking EQ response for
`+6 dB`, `500 Hz`, `Q=0.7`:

```text
H(f) = 10 log10((d^2 + (A/Q)^2) / (d^2 + 1/(A*Q)^2))
d = f/500 - 500/f
A = 10^(6/40)
```

Measured versus intended values:

| Frequency | Intended | Realized |
|---:|---:|---:|
| 100 Hz | 0.52 dB | 0.48 dB |
| 250 Hz | 2.86 dB | 2.69 dB |
| 353.6 Hz | 4.77 dB | 4.52 dB |
| 500 Hz | 6.00 dB | 5.72 dB |
| 707.1 Hz | 4.77 dB | 4.54 dB |
| 1 kHz | 2.86 dB | 2.69 dB |
| 2 kHz | 0.80 dB | 0.73 dB |

The realized peak was `5.743 dB` at approximately `503 Hz`; the analytic
peak is `6.000 dB`. The maximum sampled curve difference over 31 Hz–8 kHz
was approximately `0.59 dB`, with the main bell region approximately
`0.17–0.28 dB` below the analytic response. The curve is not materially
under-resolved or widened by the explicit points; it realizes the specified
bell closely.

The final green pilot's corrected tilt-excess measurement is `3.759 dB`.
Because the realized +6 dB/Q0.7 response matches the analytic target while
the acceptance metric remains below 4 dB, this is a specification
inconsistency between the stated bell parameters and the 4–8 dB
band-averaged acceptance criterion. No bell gain or QA threshold was
changed.

The final eight-track pilot was generated by one common serializer
orchestrator pipeline with no per-variant special cases. Its
`render_log.jsonl` records the actual `SaveProject2:` commands sent for all
eight variants:

```text
/tmp/noise-pilot-green30/render_log.jsonl
```
