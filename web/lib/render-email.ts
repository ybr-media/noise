...
export function renderFxBadges(track: LibraryTrack): string[] {
  const recipe = track.recipe;
  return fxBadges({ eq: recipe.eq ?? undefined, reverb: recipe.reverb ?? undefined });
}

export function renderFxSummary(track: LibraryTrack): string {
  return renderFxBadges(track).join(" · ") || "EQ: Flat";
}
...
function headerMarkup(appUrl: string): string {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center"><img src="${escapeHtml(`${appBase(appUrl)}/api/og/bell`)}" width="56" height="56" alt="" style="display:block;border:0" /><div style="margin-top:8px;font-size:22px;font-weight:700;color:#1c1c1e">Noise Lab</div></td></tr></table>`;
}
...
export function renderEmailHtml(input: RenderEmailInput): string {
  ...
  const trackFxBadges = renderFxBadges(first);
  ...
  const singleFxRow = !batch && trackFxBadges.length
    ? `<p style="margin:0 0 20px;text-align:center;color:#63636b;font-size:14px;line-height:22px">${escapeHtml(trackFxBadges.join(" · "))}</p>`
    : "";
  const batchTrackList = batch
