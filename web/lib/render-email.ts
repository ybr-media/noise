...
export function renderEmailHtml(input: RenderEmailInput): string {
  ...
  const image = `<img src="${escapeHtml(heroUrl(input.appUrl, first.renderKey))}" width="520" height="300" alt="${escapeHtml(`Frequency response — ${fxSummary}`)}" style="display:block;width:520px;max-width:100%;height:auto;border:0;border-radius:16px" />`;
  const lines = batch
