...
import { EQ_PRESET_GAINS, eqResponseDb } from "../lib/fx";
...
test("EQ card samples the logarithmic frequency range and flat line", () => {
  ...
  assert.ok(Math.abs(mapped[0].y - (75 - (eqResponseDb(gains, 30) / 20) * 60)) < 1e-12);
  assert.ok(Math.abs(mapped.at(-1)!.y - (75 - (eqResponseDb(gains, 16000) / 20) * 60)) < 1e-12);
});
